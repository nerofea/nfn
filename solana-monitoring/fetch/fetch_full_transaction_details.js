...
if (!signatureFile) {
  console.log('Usage: node fetch_full_transaction_details.js <signature_file> [--output <output>]');
  process.exit(1);
}
...
(async () => {
  ...
  fs.writeFileSync(outputFile, JSON.stringify(directionCounts, null, 2));
  console.log(`✅ Saved ${Object.keys(directionCounts).length} unique directions to ${outputFile}`);
})(); 